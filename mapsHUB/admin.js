function AdminApp() {
    ...
    // Localizar POIs marcados para exclusão dentro dos pontos de tour
    const pendingPOIDeletions = [];
    const pendingPOIAdditions = [];
    const pendingPOIMoves = [];

    tourPoints.forEach(pt => {
        if (pt.aiPois) {
            pt.aiPois.forEach((poi, idx) => {
                if (poi.pendingDelete) {
                    pendingPOIDeletions.push({ tourPoint: pt, poi, poiIdx: idx });
                }
                if (poi.pendingAdd) {
                    pendingPOIAdditions.push({ tourPoint: pt, poi, poiIdx: idx });
                }
                if (poi.pendingMove) {
                    pendingPOIMoves.push({ tourPoint: pt, poi, poiIdx: idx });
                }
            });
        }
    });
    
    const totalPending = pendingPlaces.length + pendingTourPoints.length + pendingPOIDeletions.length + pendingPOIAdditions.length + pendingPOIMoves.length;

    const handlePOIDeletionApproval = async (item, status) => {
        if (!confirm(`Deseja ${status === 'approved' ? 'aprovar a exclusão' : 'rejeitar e restaurar'} o local "${item.poi.name}"?`)) return;
        
        try {
            const pt = tourPoints.find(p => p.id === item.tourPoint.id);
            if (pt && pt.aiPois) {
                let newPois = [...pt.aiPois];
                if (status === 'approved') {
                    // Remove definitivamente
                    newPois = newPois.filter((_, i) => i !== item.poiIdx);
                } else {
                    // Restaura removendo a flag
                    delete newPois[item.poiIdx].pendingDelete;
                }
                await updateTourPoint(pt.id, { aiPois: newPois });
                await loadData();
            }
        } catch (e) {
            alert("Erro ao processar exclusão de local.");
        }
    };

    const handlePOIAdditionApproval = async (item, status) => {
        if (!confirm(`Deseja ${status === 'approved' ? 'aprovar a adição' : 'rejeitar e remover'} o local "${item.poi.name}"?`)) return;
        ...
        try {
            ...
            if (pt && pt.aiPois) {
                let newPois = [...pt.aiPois];
                if (status === 'approved') {
                    // Remove a flag pendingAdd e deixa ele oficial
                    delete newPois[item.poiIdx].pendingAdd;
                } else {
                    // Remove ele totalmente se rejeitado
                    newPois = newPois.filter((_, i) => i !== item.poiIdx);
                }
                await updateTourPoint(pt.id, { aiPois: newPois });
                await loadData();
            ...
            alert("Erro ao processar adição de local.");
        }
    };
            ...
            <main className="flex-1 overflow-y-auto p-4 md:p-8 pt-20 md:pt-8">
                ...
                {activeTab === 'approval' && (
                    <div className="space-y-8 animate-in fade-in">
                        ...
                        {/* Adições de POIs (Novos Locais nas fotos 360) */}
                        <div className="space-y-4">
                            <h3 className="font-bold text-gray-600 text-sm uppercase tracking-wide flex items-center gap-2">
                                <div className="icon-plus-circle"></div> Sugestões de Novos Locais 360° ({pendingPOIAdditions.length})
                            </h3>
                            
                            {pendingPOIAdditions.length === 0 ? (
                                <div className="bg-white p-8 rounded-xl text-center border border-dashed border-gray-200">
                                    <p className="text-gray-400">Nenhuma sugestão de novo local na foto.</p>
                                </div>
                            ) : (
                                <div className="grid gap-4">
                                    {pendingPOIAdditions.map((item, idx) => (
                                        <div key={idx} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                            <div>
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="bg-green-100 text-green-600 text-xs px-2 py-0.5 rounded font-bold uppercase">Novo</span>
                                                    <h4 className="font-bold text-lg text-gray-800">{item.poi.name}</h4>
                                                </div>
                                                <p className="text-sm text-gray-500">Ponto 360°: ID {item.tourPoint.id.substr(0,8)}...</p>
                                            </div>
                                            <div className="flex gap-3">
                                                <button onClick={() => handlePOIAdditionApproval(item, 'rejected')} className="px-4 py-2 rounded-lg border border-red-200 text-red-600 font-bold text-sm hover:bg-red-50 transition-colors">Rejeitar</button>
                                                <button onClick={() => handlePOIAdditionApproval(item, 'approved')} className="px-6 py-2 rounded-lg bg-green-600 text-white font-bold text-sm hover:bg-green-700 shadow-lg shadow-green-100 transition-colors">Aprovar Local</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        {/* Exclusões de POIs (Locais nas fotos 360) */}
                        <div className="space-y-4">
                            <h3 className="font-bold text-gray-600 text-sm uppercase tracking-wide flex items-center gap-2">
                                <div className="icon-trash"></div> Exclusões de Locais 360° ({pendingPOIDeletions.length})
                            </h3>
                            
                            {pendingPOIDeletions.length === 0 ? (
                                <div className="bg-white p-8 rounded-xl text-center border border-dashed border-gray-200">
                                    <p className="text-gray-400">Nenhum pedido de exclusão de local.</p>
                                </div>
                            ) : (
                                <div className="grid gap-4">
                                    {pendingPOIDeletions.map((item, idx) => (
                                        <div key={idx} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                            <div>
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="bg-red-100 text-red-600 text-xs px-2 py-0.5 rounded font-bold uppercase">Exclusão</span>
                                                    <h4 className="font-bold text-lg text-gray-800">{item.poi.name}</h4>
                                                </div>
                                                <p className="text-sm text-gray-500">Ponto 360°: ID {item.tourPoint.id.substr(0,8)}...</p>
                                            </div>
                                            <div className="flex gap-3">
                                                <button onClick={() => handlePOIDeletionApproval(item, 'rejected')} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 font-bold text-sm hover:bg-gray-50 transition-colors">Rejeitar (Restaurar)</button>
                                                <button onClick={() => handlePOIDeletionApproval(item, 'approved')} className="px-6 py-2 rounded-lg bg-red-600 text-white font-bold text-sm hover:bg-red-700 shadow-lg shadow-red-100 transition-colors">Aprovar Exclusão</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
